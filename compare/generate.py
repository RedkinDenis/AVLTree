...
bench_in_dir = to_curr_dir + "/bench"
        ...
        for j in range(split_sum) :
         ...
         #   if (k % 100000 == 0) :
           #     print("k = ", k / 100000)
            ...
            choice_request = randint(1, precision) / precision

            if (used_inserts < count_inserts and choice_request < insert_chance) :
                test_str += "k " + str(randint(1, max_number)) + "\n"
                used_inserts += 1
            else :
                if (used_ranges < count_ranges) :
                    first  = str(randint(min_number, max_number))
                    second = str(randint(min_number, max_number))
